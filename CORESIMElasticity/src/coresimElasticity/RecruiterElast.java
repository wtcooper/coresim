...
public class RecruiterElast {

	HashMap<Point, ArrayList<RecruitElast>> recruitMap = new HashMap<Point, ArrayList<RecruitElast>>(); 
	ArrayList<CoralElast> coralList; 
	ArrayList<RecruitElast> recruitList = new ArrayList<RecruitElast>(); 
	...
	private double[][] subCoverArraySimple; 
	...
	RecruiterElast (TimeKeeperElast tk) {
		this.tk = tk; 
		gridWidth = RecruitmentMainElast.getGridWidth();
		gridHeight = RecruitmentMainElast.getGridHeight(); 
		...
		survRatesArray = RecruitmentMainElast.getSurvRatesArray();
		survRatesHillArray = RecruitmentMainElast.getSurvRatesHillArray(); 
		survPropArray = RecruitmentMainElast.getSurvPropArray();
	...
	void setRecruitParams() {
		...
		larvaeMortalityDouble = tk.getLarvaeMortality(); 

		substratePref = tk.getSubstratePref(); 
		subCoverArraySimple = tk.getSubCoverArraySimple(); 
		...
		if (dispersalDist != 4){
			...
			if (dispersalDist == 0) connectivityMatrix = RecruitmentMainElast.getConnectivityMatrix0();
			else if (dispersalDist == 1) connectivityMatrix = RecruitmentMainElast.getConnectivityMatrix1();
			else if (dispersalDist == 2) connectivityMatrix = RecruitmentMainElast.getConnectivityMatrix2();
			else if (dispersalDist == 3) connectivityMatrix = RecruitmentMainElast.getConnectivityMatrix3();
		}
		...
		if (reefNumber == 0) reefCoords = RecruitmentMainElast.getCoopersReefCoords();
		else if (reefNumber == 1) reefCoords = RecruitmentMainElast.getMegansReefCoords();
		else if (reefNumber == 2) reefCoords = RecruitmentMainElast.getMk14ReefCoords();
		else if (reefNumber == 3) reefCoords = RecruitmentMainElast.getMk16ReefCoords();

		numExternalCorals = tk.getNumExternalCorals(); 
		coralArray = tk.getCoralArray();
		coralList = tk.getCoralList(); 

		recruitMap = new HashMap<Point, ArrayList<RecruitElast>>(); 
		recruitList = new ArrayList<RecruitElast>(); 		
	...
	void recruit() {
		...
		for (CoralElast coral: coralList) {
			coralX = coral.getX();
			coralY = coral.getY();
		...
		while (notSettled){
			...
			if (checkSettlement()){
				...
				if (checkPostSettlement()){
					...
					RecruitElast recruit = new RecruitElast(); 
					...
					ArrayList<RecruitElast> list = (ArrayList<RecruitElast>) recruitMap.get(point);
					if (list == null) {
						list = new ArrayList<RecruitElast>();
	...
	public HashMap<Point, ArrayList<RecruitElast>> getRecruitMap() {
		return recruitMap;
	}



	public ArrayList<RecruitElast> getRecruitList() {
		return recruitList;
	}

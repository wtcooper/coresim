...
public class CoralBuilderElast {
	...
	ArrayList<CoralElast> coralList = new ArrayList<CoralElast>();   
	...
	CoralBuilderElast(TimeKeeperElast tk) {

		reefNumber = tk.getReefNumber();

		siteCover = tk.getSiteCoverArray()[reefNumber]; 

		sizeFreqArray = RecruitmentMainElast.getSizeFreqArray();
			...
			for (int j = 0; j < numSeedsArray[i]; j++) {
				...
				while (check) {
					...
					if (checkPoint(locX, locY, size[i])){
						...
						CoralElast coral = new CoralElast();
						coral.setX(locX);
						coral.setY(locY);

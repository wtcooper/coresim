...
public class PatternRecorderElast {

	private Uniform uniform; 
	private RecruitmentMainElast rm; 

	HashMap<Point, ArrayList<RecruitElast>> recruitMap; 
	ArrayList<RecruitElast> recruitList; 
	...
	private int dispersalDist, survType, adultSpatial; // value for setting dispersal probability distribution (from beta distribution); 0-6 different options
	private double expSurvOffset, crypSurvOffset, siteSurvOffset; 
	private double larvaeMortality, siteCover, expPref, crypPref, expCover, crypCover, desperateLarvaOffset, settleMortality; 

	
	private PrintWriter outFile = null;
	
	public PatternRecorderElast(TimeKeeperElast tk, RecruitmentMainElast rm, RecruiterElast recruiter) {
		this.rm = rm; 
		...
		expSurvOffset = tk.getExpSurvOffset();
		crypSurvOffset = tk.getCrypSurvOffset();
		siteSurvOffset = tk.getSiteSurvOffset();
		larvaeMortality = tk.getLarvaeMortality();
		siteCover = tk.getSiteCoverArray()[reefNumber]; 
		expPref = tk.getSubstratePref()[0];
		crypPref = tk.getSubstratePref()[1];
		expCover = tk.getSubCoverArraySimple()[reefNumber][0];
		crypCover = tk.getSubCoverArraySimple()[reefNumber][1];
	...
	void recordTotalSitePatterns() {
		...
		for (RecruitElast recruit: recruitList) {
			...
			for (int j = 0; j < numQuadrats; j++) {
				...
				for (int k = locX-2; k <= locX+2; k++) {
					for (int l = locY-2; l <= locY+2; l++) {
						...
						ArrayList<RecruitElast> list = getRecruits(k, l);
						if (list != null) {
							for (RecruitElast recruit: list) {
								size = recruit.getDiameter(); 
								if(size < 5){ // if size is less than 5cm
									quadratAbundance++; 
	 ...
	 * @return
	 */
	ArrayList<RecruitElast> getRecruits(int x, int y){
	...
	public double getExpPref() {
		return expPref;
	}


	public double getCrypPref() {
		return crypPref;
	}


	public double getExpCover() {
		return expCover;
	}


	public double getCrypCover() {
		return crypCover;
	}

...
public class RecruiterOpti {

	HashMap<Point, ArrayList<RecruitOpti>> recruitMap = new HashMap<Point, ArrayList<RecruitOpti>>(); 
	ArrayList<CoralOpti> coralList; 
	ArrayList<RecruitOpti> recruitList = new ArrayList<RecruitOpti>(); 

	private double fecundSA = .7;  // set to 70%; description of number calculation below
	private TimeKeeperOpti tk; 
	...
	RecruiterOpti (TimeKeeperOpti tk) {
		this.tk = tk; 
		gridWidth = RecruitmentMainOpti.getGridWidth();
		gridHeight = RecruitmentMainOpti.getGridHeight(); 
		...
		subCoverArraySimple = RecruitmentMainOpti.getSubCoverArraySimple(); 
		survRatesArray = RecruitmentMainOpti.getSurvRatesArray();
		survRatesHillArray = RecruitmentMainOpti.getSurvRatesHillArray(); 
		survPropArray = RecruitmentMainOpti.getSurvPropArray();
		...
		if (dispersalDist != 4){
			...
			if (dispersalDist == 0) connectivityMatrix = RecruitmentMainOpti.getConnectivityMatrix0();
			else if (dispersalDist == 1) connectivityMatrix = RecruitmentMainOpti.getConnectivityMatrix1();
			else if (dispersalDist == 2) connectivityMatrix = RecruitmentMainOpti.getConnectivityMatrix2();
			else if (dispersalDist == 3) connectivityMatrix = RecruitmentMainOpti.getConnectivityMatrix3();
		}
	}
	...
	void setRecruitParams() {
		...
		if (reefNumber == 0) reefCoords = RecruitmentMainOpti.getCoopersReefCoords();
		else if (reefNumber == 1) reefCoords = RecruitmentMainOpti.getMegansReefCoords();
		else if (reefNumber == 2) reefCoords = RecruitmentMainOpti.getMk14ReefCoords();
		else if (reefNumber == 3) reefCoords = RecruitmentMainOpti.getMk16ReefCoords();

		numExternalCorals = tk.getNumExternalCorals(); 
		coralArray = tk.getCoralArray();
		coralList = tk.getCoralList(); 

		recruitMap = new HashMap<Point, ArrayList<RecruitOpti>>(); 
		recruitList = new ArrayList<RecruitOpti>(); 		
	...
	void setLocalSupply() {
		...
		if(uniform.nextIntFromTo(0,100) < probOfPlanulate){
			...
			numLarvaeTemp = normal.nextDouble(numLarvaePerCM, numLarvaePerCMStd)*(coralSurfArea*fecundSA*larvaeMortality);
		...
		while (notSettled){
			...
			if (checkSettlement()){
				...
				if (checkPostSettlement()){
					...
					RecruitOpti recruit = new RecruitOpti(); 
					...
					ArrayList<RecruitOpti> list = (ArrayList<RecruitOpti>) recruitMap.get(point);
					if (list == null) {
						list = new ArrayList<RecruitOpti>();
	...
	boolean checkSettlement(){
		...
		if (coralArray[larvaeX][larvaeY] != 4) {
			...
			/*				if ((autoCorr > 0.49) && (autoCorr < 0.51)){
					prob1=baseProb1;
					prob2=baseProb2;
					prob3=baseProb3;
	...
	public HashMap<Point, ArrayList<RecruitOpti>> getRecruitMap() {
		return recruitMap;
	}



	public ArrayList<RecruitOpti> getRecruitList() {
		return recruitList;
	}

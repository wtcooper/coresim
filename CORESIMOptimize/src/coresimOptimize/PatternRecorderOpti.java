...
public class PatternRecorderOpti {

	private Uniform uniform; 
	private RecruitmentMainOpti rm; 

	HashMap<Point, ArrayList<RecruitOpti>> recruitMap; 
	ArrayList<RecruitOpti> recruitList; 
	...
	public PatternRecorderOpti(TimeKeeperOpti tk, RecruitmentMainOpti rm, RecruiterOpti recruiter) {
		this.rm = rm; 
	...
	void recordTotalSitePatterns() {
		...
		for (RecruitOpti recruit: recruitList) {
			...
			for (int j = 0; j < numQuadrats; j++) {
				...
				for (int k = locX-2; k <= locX+2; k++) {
					for (int l = locY-2; l <= locY+2; l++) {
						...
						ArrayList<RecruitOpti> list = getRecruits(k, l);
						if (list != null) {
							for (RecruitOpti recruit: list) {
								size = recruit.getDiameter(); 
								if(size < 5){ // if size is less than 5cm
									quadratAbundance++; 
	...
	void recordFitness() {
		...
		// here, scale everything +10000 so that all values are positive (requirement of JGAP)
	 ...
	 * @return
	 */
	ArrayList<RecruitOpti> getRecruits(int x, int y){
